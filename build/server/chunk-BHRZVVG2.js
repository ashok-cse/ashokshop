...
var typingTimer;
var send = async ({ method, path, params, data, token, headers }) => {
  var _a, _b;
...
var getAPI = (path, headers) => {
  return send({ method: "GET", path, headers });
};
var post = (path, data, headers) => {
  return send({ method: "POST", path, data, headers });
};
var startDelayedLoadingIndicator = async () => {
  clearTimeout(typingTimer);
  typingTimer = setTimeout(() => {
    loadingDelayed.set(true);
  }, 5e3);
};
var cancelDelayedLoadingIndicator = async () => {
  clearTimeout(typingTimer);
...
//# sourceMappingURL=chunk-BHRZVVG2.js.map
